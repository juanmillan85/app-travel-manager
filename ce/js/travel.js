        ...
        for (var j = 0, jend = layoutDay.pois.length; j < jend; j++) {
            ...
            if (action == 'edit') {
                ...
                if (poi.id != 'deleted') {
                    ...
                    frardiv.appendChild(frupar);
                    frardiv.appendChild(frdoar);
                }
            }
            ...
            frdatadiv.appendChild(frh4);
            frdatadiv.appendChild(frp);
            frdatadiv.appendChild(frnotes);
...
TravelGeneral.drawTravelList = function (list) {
    ...
    for (var i = 0, iend = travels.length; i < iend; i++) {
        ...
        if (listtype == 'anchors') {
            ...
            trlstitle.appendChild(trlsinp);
            trlstitle.appendChild(trlstitletext);
        }
        trlsli.appendChild(trlsdate);
        trlsli.appendChild(trlstitle);
...
TravelEditor.movePOI = function (dayArrayIndex, POIarrayIndex, direction) {
    ...
    if (direction == "up") {
        ...
        POIboundary = 0;
        changeDay = function () {
            TravelGeneral.layout[dayArrayIndex - 1].pois.push(tmp);
        }
        nudgePOI = function () {
            TravelGeneral.layout[dayArrayIndex].pois.splice(POIarrayIndex - 1, 0, tmp);
        }
    } else if (direction == "down") {
        dayBoundary = TravelGeneral.layout.length - 1;
        POIboundary = TravelGeneral.layout[dayArrayIndex].pois.length - 1;
        changeDay = function () {
            TravelGeneral.layout[dayArrayIndex + 1].pois.splice(0, 0, tmp);
        }
        nudgePOI = function () {
            TravelGeneral.layout[dayArrayIndex].pois.splice(POIarrayIndex + 1, 0, tmp);
        }
    ...
    $('.thumbs img').each(function () {
        $(this).bind('click', function (e) {
            ...
            pictdiv.show("fade", {}, 500, function () {
                window.scrollTo(0, 1);
                /*fix for weird android scroll behavior when overflow:hidden*/
                pictdiv.css('top', 0);
            });
        });
    });
...
var POISearch = function (paramFieldId, paramPoiSourceId, paramPoiNotFoundId) {
    ...
    for (k in wt_pois) {
        ...
        google.maps.event.addListener(mrk, 'click', function () {
            document.location.href = 'poi.html?id=' + wt_pois[k].id;
        });
        mrksArray.push(mrk);
    }
    ...
    this.conductSearch = function () {
        ...
        if (result.length == 0) {
            ...
            for(i in result)       {
                var openpoint=result[i];
                var name =openpoint.display_name
